...
procdump(void)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    printf("%d %s %s Priority : %d, Times executed : %d, Last time executed : %d", p->pid, state, p->name,p->priority,p->times_executed,p->last_executed);
    printf("\n");
  }
}
...
int pstat(int pid) {
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->pid == pid) {
        printf("Priority : %d, Times executed : %d, Last time executed : %d",p->priority, p->times_executed,p->last_executed);
        release(&p->lock);
        break;
      }
      release(&p->lock);
