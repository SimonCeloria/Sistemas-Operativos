...
static void read_cluster_dir_entries(u8 *buffer, fat_dir_entry end_ptr,
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        if (ignore_dentry(disk_dentry_ptr)) { // fs.log      (0xe5)s -> base_name      log -> extension
            if (!((char)disk_dentry_ptr->base_name[0] == (char)0xe5 && disk_dentry_ptr->base_name[1] == 's' &&
                ((char*)disk_dentry_ptr->extension)[0] == 'l' && ((char*)disk_dentry_ptr->extension)[1] == 'o' &&
...
void fat_file_unlink(fat_file file, fat_file parent) {
    u32 next_cluster =0, last_cluster =0;
    file->dentry->base_name[0] = FAT_FILENAME_DELETED_CHAR;

    //Cambiar entrada del directorio
    next_cluster = fat_table_get_next_cluster(file->table,file->start_cluster);
    //Seteo el primer cluster en libre
    ...
    //Marco los siguientes cluster como libres
    while(!fat_table_is_EOC(file->table, last_cluster)) {
        next_cluster = fat_table_get_next_cluster(file->table,last_cluster);
        fat_table_set_next_cluster(file->table,last_cluster,FAT_CLUSTER_FREE);
        last_cluster = next_cluster;
